...
1.2.3.更改了 *update_song_list()* 的部分代码描述.\
1.2.4.将UI窗口搭建 *build_platform()* 与窗口居中且固定大小 *center()* 的调用绑定到 *APP(<class 'type'>)* 对象的构造函数 *(__init__)* 中. 
#### 2023/7/3
...
|  *"play_dict"*  |
#### 2023/11/10
1.14.10.将本地音乐文件夹路径配置设置在 *PlayerConfig.json* 文件 \
主要变动: 
|新增key|
|---|
|  *music_folders_path*  |
#### 2023/11/11
2.0.0.对UI界面进行了重构 \
主要变动: 使用PyQt5模块在保持原功能不变的基础上重新构建了所有UI界面.
