...
#### 2.16.20
为了便于本地打包项目,将程序入口(main)置于源码包(source)外.\
优化了部分数据初始化方法.\
优化了设置-配置文件界面"打开文件"按钮的方法(动态获取绝对路径).

#### 2.16.19
在设置-背景图界面添加了初始化按钮,用于将全部可更改的图片图标恢复默认设置.
为设置-背景图界面的"更改图片", "更改图标"按钮同样追加了预览图片/图标刷新的方法.

#### 2.15.19
在设置-快捷键界面添加了初始化按钮,用于清空快捷键设置.\
添加了配置数据的初始化检测方法.

#### 2.14.19
在设置-歌单界面添加了初始化按钮,用于播放列表的导入.

#### 2.13.19
修复了设置-歌单界面,"移动到其他歌单","复制到其他歌单"两个按钮操作的逻辑问题.

#### 2.13.18
在设置-歌单界面添加了"创建新歌单","创建新歌单"两个按钮.\
进行了代码规范--对部分变量名进行调整.
...
#### 2.11.18
优化了歌曲搜索过程, 以及歌曲搜索界面树形图与APP界面的文本显示规则.

#### 2.11.17
修复了切换歌单后, 播放序号与位置异常的情况.现改为从切换后的歌单第一首的第0秒开始.

#### 2.11.16
修复了部分情况下,歌单设置界面歌单项下拉列表不显示文本的问题.\
修复了部分音频类型的文件无法显示和播放的问题.

#### 2.11.15
将歌单的切换功能移动至设置UI界面,新增了歌单的显示.

#### 2.11.14
新增了自定义快捷键界面组件可选中性的的变化.
...
#### 2.10.13
将键盘快捷方式的切换移至设置UI界面, 新增了自定义快捷键功能, 仅能够储存一套自定义方案.

#### 2.9.13
将配置文件的打开操作移至设置UI界面.

#### 2.9.12
添加了设置UI界面, 并新增更换窗口背景图/图标功能. 其余部分功能将逐步移入设置UI.

#### 2.8.12
将MyWidgetMethod.py模块名变更为Simple_Qt.py,更改了部分方法名称,并优化了部分方法实现以及描述.\
添加了对布局(Layout)的简单集成.\
同时,Simple_Qt.py模块的维护更新将独立出该项目,且不再出现在日志中.
