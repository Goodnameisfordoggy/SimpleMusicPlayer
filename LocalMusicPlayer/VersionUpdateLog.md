...
1.1.5.将UI窗口搭建*build_platform()* 与 窗口居中且固定大小*center()*的调用默认到*APP(<class 'type'>)*对象的构造函数*(__init__)*中.\
...
#### 2023/7/12
3.1.2.优化了代码结构\
主要变动: 将 一级菜单*更改文件夹*的所有属性与方法从*APP(<class 'type'>)*中剥离,并整合到一个新的局部类*ChangeFolderMenu(<class 'type'>)*中.
#### 2023/9/16
3.2.1.优化了代码结构\
...
#### 2023/10/7
4.0.1.添加了本地数据保存功能.\
原理: 使用*json*文件进行部分底层数据的初始化,以及一些关键数据的储存.
| 新增key|
|----|
|  *"music_folder_path"*  |
